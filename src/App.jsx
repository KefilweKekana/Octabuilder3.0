...
import { Save, Grid, Layout, Settings, Home, FileText, Plus, X, GripVertical, Check, Download, Upload, RefreshCw, List } from 'lucide-react';
...
export default function ERPFlowAppBuilder() {
  ...
  const [currentView, setCurrentView] = useState('home'); // 'home', 'forms'
  ...
  useEffect(() => {
    ...
    const savedSecret = localStorage.getItem('api_secret');
    const saved = localStorage.getItem('app_config');

    if (savedUrl) setErpnextUrl(savedUrl);
    if (savedKey) setApiKey(savedKey);
    if (savedSecret) setApiSecret(savedSecret);
    if (saved) setAppConfig(JSON.parse(saved));

    if (savedUrl && savedKey && savedSecret) {
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-20 bg-white border-r flex flex-col items-center py-6 space-y-4">
          ...
          <button
            ...
            className="w-12 h-12 rounded-lg flex items-center justify-center bg-gray-100 hover:bg-gray-200"
            title="Forms"
          >
